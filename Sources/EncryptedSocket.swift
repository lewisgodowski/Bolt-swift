...
public class EncryptedSocket: UnencryptedSocket {
    public lazy var certificateValidator = UnsecureCertificateValidator(
        hostname: hostname,
        port: UInt(port)
    )

#if os(Linux)
    override func setupBootstrap(
        _ group: MultiThreadedEventLoopGroup,
        _ dataHandler: ReadDataHandler
    ) -> (Bootstrap) {
    ...
    override func setupBootstrap(
        ...
        return NIOTSConnectionBootstrap(group: group)
            ...
            .tlsConfig(validator: certificateValidator)
    }
#endif
}
...
extension NIOTSConnectionBootstrap {
    func tlsConfig(validator: CertificateValidatorProtocol) -> NIOTSConnectionBootstrap {
        ...
        return tlsOptions(options)
    }

    func tlsConfigDefault() -> NIOTSConnectionBootstrap {
        tlsOptions(.init()) // To remove TLS (unencrypted), just return self
    }
}
#endif
